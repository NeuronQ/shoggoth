...
func Seq(parsers []Parser, resultName string, name string) Parser {
	return func(input string, pos int, makeResult ASTreeMaker) (newPos int, result interface{}, rerr error) {
		...
		for _, sp := range parsers {
			nextPos, r, err := sp(input, lastPos, makeResult)
			if err != nil {
		...
		if resultName != "" {
			result = makeResult(resultName, results...)
		}
		return lastPos, result, nil
	}
}
